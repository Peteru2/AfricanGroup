...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="nav  font-roboto" ref={navbarRef}>
        <div className="flex navPad">
          ...
          <div className={name ? "nav-menu   active" : "nav-menu "}>
                ...
                <li className="nav-list" key={index}>
                  <Link className={`flex w-full`} to={item.url}>
                    <motion.span
                      ...
                      className={`text-[19px] md:text-[17px] ${
                        isActive ? "text-private font-bold" : ""
                      }`}
                    >
                      {item.title}
                    </motion.span>
            ...
            <Link to={"/request-quote"}>
              <div className="xl:ml-auto flex text-white w-44 group items-center bg-private hover:bg-public hover:bg-opacity-90   transition ease-in-out delay-150 duration-900 rounded-lg">
                <button className=" text-xs flex items-center justify-center rounded-lg w-full">
                  <span className="p-2 w-full flex justify-center">
                    REQUEST A QUOTE
                  </span>
                  <i className="bg-public bg-opacity-50 ml-auto p-3 transition ease-in-out delay-150 duration-900 group-hover:bg-private rounded-tr-lg rounded-br-lg fa fa-chevron-right"></i>
                </button>
              </div>
            </Link>
          </div>
          ...
          <div className="menu-icon ml-auto items-center" onClick={handleClick}>
            <i
              className={
                name
                  ? "fa fa-times items-center text-private"
                  : "fa fa-bars-staggered items-center"
              }
            ></i>
          </div>
          <div></div>
        ...
        <Link to={url}>
          <div>
            <h2 className="bg-private bg-opacity-60 text-white font-light py-2 text-center">
              {content}
            </h2>
          </div>
        </Link>
